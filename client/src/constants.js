export const IMGAE_BASE_URL = 'http://localhost:5002/uploads'
export const BASE_URL = 'http://localhost:5002/api'

export const TIMELINE_URL = `${BASE_URL}/posts/timeline`
export const PROFILE_POSTS = `${BASE_URL}/posts/profile`
export const LOGIN = `${BASE_URL}/auth/login`
export const SIGNUP = `${BASE_URL}/auth/signup`
export const NEW_POST_URL = `${BASE_URL}/posts`
export const UPLOAD_IMAGE_URL = `${BASE_URL}/upload`